...
@interface rcvColorController : PSEditableListController {}
@end
 
@implementation rcvColorController
	- (id)specifiers{
		if (!_specifiers){
			NSMutableArray *spec=[[NSMutableArray alloc] init];
			
			PSSpecifier *groupSpec=[PSSpecifier groupSpecifierWithName:@""];
			[groupSpec setProperty:@"Click the edit/+ button to add hex colors. Swipe or enter editing mode to delete colors." forKey:@"footerText"];
			[spec addObject:groupSpec];
			
			// Load custom colors
			CFPreferencesAppSynchronize(CFSTR("com.chewmieser.messageColors"));
			NSData *dat=(__bridge NSData *)CFPreferencesCopyAppValue(CFSTR("rcvColorArray"),CFSTR("com.chewmieser.messageColors"));
	...
	- (void)setEditable:(BOOL)arg1{
		...
		if (arg1==YES){
			...
			// Save prefs
			CFPreferencesSetValue(CFSTR("rcvColorArray"), (__bridge CFPropertyListRef)[NSKeyedArchiver archivedDataWithRootObject:[self getColorList]], CFSTR("com.chewmieser.messageColors"), kCFPreferencesCurrentUser, kCFPreferencesAnyHost);
			CFPreferencesAppSynchronize(CFSTR("com.chewmieser.messageColors"));
