...
@interface messageColorsHeader : PSTableCell {
	UILabel *_label;
}
@end
 
@implementation messageColorsHeader
	- (id)initWithSpecifier:(PSSpecifier *)specifier {
		...
		if (self) {
			...
			NSMutableAttributedString *attributedString=[[NSMutableAttributedString alloc] initWithString:@"messageColors"];
			[attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor grayColor] range:NSMakeRange(0, 7)];
			[attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor redColor] range:NSMakeRange(7, 6)];
